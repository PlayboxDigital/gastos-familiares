...
import { Activity, Package, ShoppingCart,  Edit2, Trash2 } from 'lucide-react';
import { consumoInteligenteService } from '@/services/consumoInteligente';
import { ConsumoBaseFormValues, ConsumoBaseItem } from '@/types';
...
const PRIORITY_OPTIONS = ['Esencial', 'Importante', 'Prescindible'] as const;
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
        ...
        <Button className="w-full sm:w-auto" onClick={openAddProduct}>
      ...
      <Card className="rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <CardContent className="p-4 sm:p-5">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-4">
                      ...
                      <TableBody>
                        {items.map((item) => (
                          <TableRow key={item.consumo_base_id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <TableCell className="px-4 py-4 text-right text-slate-700">{item.duracionMeses}</TableCell>
            ...
            {tabDefinitions.filter((tab) => tab.value !== 'consumo-base').map((tab) => (
